...
- **No transparency, for candidates**: candidates apply into a void, with no feedback, no signal,
  no idea where they fell short. The silence is demoralising and uninformative.
...
something to write answers to. Here's a glimpse of the scoring structure it's built from:
...
- **A real job seeker's experience**: we spoke to someone applying into AI safety about where
  the process breaks down, with no feedback, no signal, no idea where they fell short. That pain
...
From a pool of roughly 6,000 candidates, sequential gates narrowed the field: location (61% passed), ops
experience (33% passed), a systems-building signal (96% passed). A
